...
class TestGlobalization(GlobalizationTests):
    test_charset_data = [
        ('double',
            (
    ...
    @pytest.mark.parametrize("charset_data", test_charset_data)
    def test_charset(self, charset_data, client):
        """
            Verify the column names and string values in rows returned by
            select statement are properly encoded as unicode.
        """
        _, charset = charset_data
    ...
    @pytest.mark.parametrize("charset_data", test_charset_data)
    def test_schema_metadata(self, charset_data, client):
        self.run_schema_metadata_validation(charset_data[1], client)

    @pytest.mark.parametrize("charset_data", test_charset_data)
    def test_table_metadata(self, charset_data, client):
        self.run_schema_metadata_validation(charset_data[1], client)

    def run_schema_metadata_validation(self, charset, client):
        new_schemas = []
